...
func (suite *StatsSuite) TestStats(c *C) {
	connection := OpenConnection("stats-conn", "tcp", "192.168.99.100:6379", 1)
	c.Assert(NewCleaner(connection).Clean(), IsNil)

	conn1 := OpenConnection("stats-conn1", "tcp", "192.168.99.100:6379", 1)
	conn2 := OpenConnection("stats-conn2", "tcp", "192.168.99.100:6379", 1)
